...
const generatePDFPage = (doc, copyType, invoiceNumber, logoBase64) => {
  ...
  // Logo
  doc.addImage(logoBase64, 'JPEG', startX, headerStartY + 1, 27, 27);

  // Header Text
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8.5);
  doc.setTextColor(255, 0, 0);
  doc.text('SENTHILKUMAR FIREWORK INDUSTRIES', startX + 30, headerStartY + 5);
  ...
  doc.setFontSize(9.5);
  doc.setTextColor(255, 0, 0);
  doc.text('WAY BILL', 150, headerStartY + 5);
  doc.setFontSize(9);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(0, 0, 0);
  doc.text(`Invoice Number: SKFI-${invoiceNumber}-25`, 150, headerStartY + 2 + 2 * lineSpacing);
  ...
  tableBody.push(
    ...
    [{ content: 'Despatched From:', colSpan: 4, styles: { halign: 'right', fontStyle: 'bold', fillColor: '#fff' } }, { content: despatchedFrom || 'N/A', colSpan: 4, styles: { fillColor: '#fff' } }],
    [{ content: 'Despatched To:', colSpan: 4, styles: { halign: 'right', fontStyle: 'bold', fillColor: '#fff' } }, { content: despatchedTo || 'N/A', colSpan: 4, styles: { fillColor: '#fff' } }],
    [{ content: 'Transport Name:', colSpan: 4, styles: { halign: 'right', fontStyle: 'bold', fillColor: '#fff' } }, { content: transportName || 'N/A', colSpan: 4, styles: { fillColor: '#fff' } }]
  );
...
const CustomerCopy = async () => {
  ...
  const doc = new jsPDF();
  generatePDFPage(doc, 'Customer COPY', invoiceNumber, MyLogo);
